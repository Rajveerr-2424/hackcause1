...
function App() {
  ...
  const openFleetModal = async () => {
    try {
      ...
      alert('Could not fetch fleet data.');
    }
  };

  // Group fleet by state
  const fleetByState = fleetData.reduce((acc, t) => {
    if (!acc[t.state]) acc[t.state] = { available: 0, total: 0 };
    acc[t.state].total += 1;
    if (t.is_available) acc[t.state].available += 1;
    return acc;
  }, {});
        ...
        <div className="w-full pointer-events-auto bg-gradient-to-t from-slate-900/95 via-slate-900/80 to-transparent pb-6 pt-24 px-8 backdrop-blur-sm">
          ...
          <div className="flex justify-between items-end mb-6">
            ...
            <div className="flex gap-4">
              ...
              <div
                className="bg-white/10 border border-white/10 rounded-xl px-4 py-2 flex items-center gap-3 backdrop-blur-md cursor-pointer hover:bg-white/20 transition"
        ...
        {showFleetModal && (
          ...
          >
            <motion.div
              ...
              className="bg-slate-900/95 border border-white/10 rounded-3xl shadow-2xl w-[420px] max-h-[80vh] flex flex-col overflow-hidden"
            >
              {/* Modal Header */}
              <div className="p-6 border-b border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-blue-500/20 rounded-xl"><Truck className="text-blue-400" size={22} /></div>
                  <div>
                    <h3 className="text-lg font-bold text-white">National Tanker Fleet</h3>
                    <p className="text-xs text-slate-400">{availableTankers} of {fleetData.length} available</p>
                  </div>
                </div>
                <button onClick={() => setShowFleetModal(false)} className="p-2 bg-white/5 hover:bg-white/10 rounded-full text-slate-400 transition">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {Object.entries(fleetByState).map(([state, counts]) => (
                  <div key={state} className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl px-4 py-3">
                    <div>
                      <p className="text-white font-semibold text-sm">{state}</p>
                      <p className="text-slate-500 text-xs">{counts.available} available / {counts.total} total</p>
                    </div>
                    <div className="flex gap-1">
                      {Array.from({ length: counts.total }).map((_, i) => (
                        <div
                          key={i}
                          className={`w-3 h-3 rounded-full ${i < counts.available ? 'bg-emerald-400' : 'bg-red-500 opacity-50'}`}
                          title={i < counts.available ? 'Available' : 'Deployed'}
                        />
                      ))}
                    </div>
                  </div>
              ...
              {/* Legend */}
              <div className="p-4 border-t border-white/10 flex gap-4 text-xs text-slate-400">
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-emerald-400 inline-block" /> Available</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-red-500 opacity-50 inline-block" /> Deployed</span>
              </div>
            </motion.div>
          </motion.div>
        )}
